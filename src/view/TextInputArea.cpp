...
TextInputArea::TextInputArea(ftxui::Component input_component, ftxui::Component text_component) :
    input_component_(std::move(input_component)), text_component_(std::move(text_component)),
    session_end_component_(ftxui::Renderer({[] { return ftxui::text("Session Finished!!!"); }})),
    main_component_(ftxui::Container::Stacked(
            {ftxui::Maybe(ftxui::Container::Stacked({text_component_, input_component_}),
...
ftxui::Component TextInputArea::get_text_input_component() const {
    return ftxui::Renderer(input_component_, [&] {
        ...
        return ftxui::flexbox({main_element | ftxui::center | ftxui::border | size(ftxui::HEIGHT, ftxui::EQUAL, 10) |
