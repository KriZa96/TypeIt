...
InputLineEngine::InputLineEngine(const Text& text_instance) :
    ...
    total_input_lines_(static_cast<std::size_t>(text_instance_.get_text_lines_size() + 1),
                       ftxui::hbox(ftxui::text(""))) {}


[[nodiscard]] int InputLineEngine::get_previous_lines_size() const {
    return std::accumulate(previous_input_lines_.begin(), previous_input_lines_.end(), 0,
                           [](const int sum, const ftxui::Elements& line) {
                               return sum + static_cast<int>(std::distance(line.begin(), line.end()));
                           });
}
...
void InputLineEngine::go_to_new_line() {
    ...
    if (current_line_index_ < static_cast<std::size_t>(text_instance_.get_text_lines_size() - 1)) {
        FocusPosition::y++;
    }
}
...
bool InputLineEngine::should_go_to_next_line(char next_character) const {
    return next_character == ' ' &&
           current_line_index_ < static_cast<std::size_t>(text_instance_.get_text_lines_size() - 1) &&
           current_input_line_.size() >=
                   static_cast<std::size_t>(text_instance_.get_text_line_size(static_cast<int>(current_line_index_)));
}


bool InputLineEngine::should_finish_game() {
    return current_line_index_ + 1 >= static_cast<std::size_t>(text_instance_.get_text_lines_size()) &&
           current_input_line_.size() + 1 >=
                   static_cast<std::size_t>(text_instance_.get_text_line_size(static_cast<int>(current_line_index_)));
}
...
void InputLineEngine::go_to_previous_line() {
    ...
    if (current_line_index_ < static_cast<std::size_t>(text_instance_.get_text_lines_size() - 2)) {
        FocusPosition::y--;
    }
}
...
void InputLineEngine::remove_element(std::size_t input_text_size) {
    if (input_text_size < static_cast<std::size_t>(get_previous_lines_size())) {
        go_to_previous_line();
        return;
    }
...
bool InputLineEngine::should_add_element(std::size_t input_text_size) const {
    const size_t previous_elements_size =
            static_cast<std::size_t>(get_previous_lines_size()) + current_input_line_.size();
...
[[nodiscard]] ftxui::Element InputLineEngine::get_next_character(const char next_character) {
    ...
    if (next_character ==
        text_instance_.get_char_at_line_and_position(static_cast<int>(current_line_index_),
                                                     std::max(static_cast<int>(current_input_line_.size()), 0))) {
