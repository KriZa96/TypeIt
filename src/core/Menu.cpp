...
Menu::Menu(ftxui::ScreenInteractive& screen) :
    screen_(screen), text_radiobox_choice_({"simple", "medium", "hard", "custom"}),
    time_radiobox_choice_({"15s", "30s", "60s", "custom"}),
    path_input_(ftxui::Input(&GameOptions::text_radiobox_values_[3], "Path to file",
                             ComponentOptions::menu_path_input_option)),
    time_input_(ftxui::Input(&GameOptions::custom_radiobox_input_, "Input seconds",
                             ComponentOptions::menu_time_input_option)),
    time_radiobox_(ftxui::Radiobox(&time_radiobox_choice_, &GameOptions::selected_radiobox_time_,
...
ftxui::Component Menu::get_menu_component() const {
    return ftxui::Renderer(menu_container_, [this] { return menu_container_->Render(); });
}


ftxui::Component Menu::get_time_radiobox_component_() const {
    return ftxui::Renderer(time_radiobox_,
                           [this] { return ftxui::window(ftxui::text("Time"), time_radiobox_->Render()); });
}


ftxui::Component Menu::get_text_radiobox_component_() const {
    return ftxui::Renderer(text_radiobox_,
                           [this] { return ftxui::window(ftxui::text("Text"), text_radiobox_->Render()); });
}


ftxui::Component Menu::get_info_component_() const {
    return ftxui::Maybe(ftxui::Renderer([&] {
                            return ftxui::window(ftxui::text("Info"),
                                                 ftxui::paragraph("Custom inputs will change color from "
...
ftxui::Component Menu::get_menu_container_() const {
    return ftxui::Container::Vertical(
                   {get_info_component_(), ftxui::Container::Horizontal({info_button_}) | ftxui::center,
                    ...
                    ftxui::Container::Horizontal({start_button_, exit_button_}) | ftxui::center}) |
           Style::menu_style;
}
