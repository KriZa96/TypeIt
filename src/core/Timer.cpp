...
int Timer::get_elapsed_time() {
    ...
    elapsed_time_ = static_cast<int>(
            std::chrono::duration_cast<std::chrono::seconds>(std::chrono::steady_clock::now() - start_time_).count());
    return elapsed_time_;
}


std::string Timer::get_time_left_str() { return std::format("{}s", std::max(0, total_time_ - get_elapsed_time())); }


ftxui::Element Timer::get_time_element() { return ftxui::text(get_time_left_str()); }


ftxui::Component Timer::get_time_component() {
    return ftxui::Renderer([&] { return get_time_element(); });
}


int& Timer::get_elapsed_time_reference() { return elapsed_time_; }


void Timer::start_timer() { started_timer_ = true; }
