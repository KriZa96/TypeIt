...
ftxui::Component Input::get_input_component() {
    return ftxui::Renderer(input_component_,
                           [&] {
                               input_line_.render_input_text(input_text_.empty() ? ' ' : input_text_.back(),
                                                             input_text_.size());
                               input_word_count_.set_word_count(input_text_);
                               return ftxui::vbox(input_line_.get_total_input_lines()) |
                                      ftxui::focusPosition(FocusPosition::x, FocusPosition::y) | ftxui::frame |
...
ftxui::Component Input::get_accuracy_component() const {
    return ftxui::Renderer(
            [this] { return ftxui::text(std::format("acc: {:.1f}%", input_line_.get_percentage_of_correct_input())); });
}
