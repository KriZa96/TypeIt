...
namespace typeit::core {
    ...
    TypingModel::TypingModel(const TextBuffer& target) :
        target_{target.graphemes()}, states_(target.size(), GraphemeState::Pending), ever_wrong_(target.size(), false) {
    }
    ...
    void TypingModel::type(const Grapheme& grapheme, Millis at) {
        ...
        if (is_word_separator(grapheme) && !is_word_separator(target_[cursor_])) {
